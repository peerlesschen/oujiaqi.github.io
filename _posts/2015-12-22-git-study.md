...
* 第一步，使用命令`git add <file>`，注意，可反复使用添加多个文件，添加所有用`*`
...
合并分支到当前分支：`git merge <name>` 即将当前的`HEAD`指针指向指定的分支处，可以是`Fast-forward`“快进模式”，也可以是其他模式。

删除分支：`git bransh -d <name>` 合并完成后再删除
...
2. 切换到`master`分支，同样编辑"readme.txt"文件，添加提交，成为这样了  ![](/pictures/git6.png)
3. 试图合并，不成功，产生冲突，git不知道你要的是那一份。
4. `git status` 可以告诉我们冲突的文件，然后我们直接查看冲突文件的内容，并编辑。`<<<<<<<<`，`=========`，`>>>>>>>>>`标记处不同分支的内容，我们修改后保存，再添加提交就变成了这样了  ![](/pictures/git7.png)
...
本地新建的分支如果不推送到远程，对其他人就是不可见的，推送远程库，都是分支对分支的
...
配置命令别名`git config --global alias.st status`把`st`替换`status`。
